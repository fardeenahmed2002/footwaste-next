...
export default function Page() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      form.append("donorof", formdata.donorof)
      form.append("avatar", avatar)
      axios.defaults.withCredentials = true;
      const { data } = await axios.post("http://localhost:3000/api/auth/signup/donor", form, {
        headers: {
      ...
      <motion.div
        ...
        <div className='flex flex-col justify-center items-center'>
          ...
          <div className="flex space-x-4">
            <label className="flex items-center gap-1">
              <input type="radio" name="role" onClick={() => router.push('/signup')} />
              <span>User</span>
            </label>
            <label className="flex items-center gap-1 cursor-pointer" >
              <input defaultChecked type="radio" name="role" />
              <span>Donor</span>
            </label>
        ...
        <form className="space-y-4 w-full" onSubmit={handleSubmit}>
          ...
          <div className="flex gap-4">
            ...
            <div className="w-1/2"><label className="block text-sm font-medium">Donor of</label>
              <input
                type="text"
                name="donorof"
                value={formdata.donorof}
                onChange={handleChange}
                placeholder="Hotel, Restaurants, House etc ....  "
                className="w-full px-4 py-2 mt-1 bg-white/20 text-white placeholder-white rounded-md border border-white/30 focus:outline-none"
              />
            </div>
          </div>
