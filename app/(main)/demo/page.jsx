...
const DonationPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!donorType || !amount) {
      alert(inEng ? "Please fill all required fields!" : "সব ফিল্ড পূরণ করুন!");
      return;
    }
    ...
    try {
      ...
      if (!saveResponse.data.success) {
        alert(inEng ? "Failed to save donation!" : "দান সংরক্ষণ ব্যর্থ হয়েছে!");
        return;
      }

      // ২. Create SSL payment session
      ...
      if (paymentResponse.data.success && paymentResponse.data.url) {
        ...
        alert(inEng ? "Payment URL not found!" : "পেমেন্ট URL পাওয়া যায়নি!");
      }

    } catch (err) {
      console.error(err);
      alert(inEng ? "Something went wrong!" : "কিছু ভুল হয়েছে!");
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 bg-white rounded-md shadow-md mt-10">
      ...
      <p className="mt-2 text-gray-600 text-sm text-center md:col-span-4">
        {inEng
          ? "Donate to As-Sumaih Foundation for tax benefits."
          : "আস-সুমাইহ ফাউন্ডেশনে দান করলে কর রেয়াত পাবেন।"}{" "}
        <span className="text-green-600 underline cursor-pointer">
          {inEng ? "Click here for details." : "বিস্তারিত জানতে ক্লিক করুন।"}
        </span>
      </p>
    </div>
  );
